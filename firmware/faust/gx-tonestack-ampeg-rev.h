...
Compilation options: -lang cpp -i -fpga-mem-th 4 -ct 1 -cn GxTonestackAmpegRev -scn chdsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -double -ftz 0
------------------------------------------------------------ */

#ifndef  __GxTonestackAmpegRev_H__
#define  __GxTonestackAmpegRev_H__
...
#ifndef FAUSTCLASS 
#define FAUSTCLASS GxTonestackAmpegRev
#endif
...
static double GxTonestackAmpegRev_faustpower2_f(double value) {
	return value * value;
}
static double GxTonestackAmpegRev_faustpower3_f(double value) {
	return value * value * value;
}

class GxTonestackAmpegRev : public chdsp {
 ...
 public:
	...
	GxTonestackAmpegRev(const GxTonestackAmpegRev&) = default;
	
	virtual ~GxTonestackAmpegRev() = default;
	
	GxTonestackAmpegRev& operator=(const GxTonestackAmpegRev&) = default;
	
	void metadata(Meta* m) { 
		m->declare("chameleon_flags", "-double");
		m->declare("compile_options", "-lang cpp -i -fpga-mem-th 4 -ct 1 -cn GxTonestackAmpegRev -scn chdsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -double -ftz 0");
		m->declare("filename", "gx-tonestack-ampeg-rev.dsp");
	...
	virtual void instanceConstants(int sample_rate) {
		...
		fConst1 = GxTonestackAmpegRev_faustpower2_f(fConst0);
		fConst2 = GxTonestackAmpegRev_faustpower3_f(fConst0);
		fConst3 = 3.0 * fConst2;
	}
	...
	virtual GxTonestackAmpegRev* clone() {
		return new GxTonestackAmpegRev(*this);
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("Rev.Rocket");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		double fSlow2 = 4.7047000000000006e-07 * fSlow1;
		double fSlow3 = fConst1 * (fSlow1 * (1.1761750000000001e-05 * fSlow0 + -4.217780000000001e-06 - fSlow2) + 0.00011849250000000002 * fSlow0 + 5.107200000000001e-06);
		double fSlow4 = 2.9375000000000002e-09 * fSlow0;
		double fSlow5 = 4.1125e-10 * fSlow0 - 1.645e-11 * fSlow1;
		double fSlow6 = fConst2 * (fSlow1 * (fSlow5 + -1.0105e-10) + fSlow4 + 1.175e-10);
		double fSlow7 = 0.025025000000000002 * fSlow0;
		double fSlow8 = 0.00047000000000000004 * fSlow1;
		double fSlow9 = fConst0 * (fSlow8 + fSlow7 + 0.015726);
		...
		double fSlow16 = fSlow1 * (fSlow5 + 1.645e-11) + fSlow15 * (fSlow4 + 1.175e-10 * (1.0 - fSlow1));
		double fSlow17 = fConst2 * fSlow16;
		double fSlow18 = fConst1 * (3.675000000000001e-07 * fSlow15 + fSlow1 * (4.8222e-07 - fSlow2) + fSlow0 * (1.1761750000000001e-05 * fSlow1 + 9.925e-07) + 3.9700000000000005e-08);
		double fSlow19 = fConst0 * (fSlow7 + fSlow8 + 2.5e-05 * fSlow15 + 0.001001);
