...
Compilation options: -lang cpp -i -fpga-mem-th 4 -ct 1 -cn GxTonestackRoland -scn chdsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -double -ftz 0
------------------------------------------------------------ */

#ifndef  __GxTonestackRoland_H__
#define  __GxTonestackRoland_H__
...
#ifndef FAUSTCLASS 
#define FAUSTCLASS GxTonestackRoland
#endif
...
static double GxTonestackRoland_faustpower2_f(double value) {
	return value * value;
}
static double GxTonestackRoland_faustpower3_f(double value) {
	return value * value * value;
}

class GxTonestackRoland : public chdsp {
 ...
 public:
	...
	GxTonestackRoland(const GxTonestackRoland&) = default;
	
	virtual ~GxTonestackRoland() = default;
	
	GxTonestackRoland& operator=(const GxTonestackRoland&) = default;
	
	void metadata(Meta* m) { 
		m->declare("chameleon_flags", "-double");
		m->declare("compile_options", "-lang cpp -i -fpga-mem-th 4 -ct 1 -cn GxTonestackRoland -scn chdsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -double -ftz 0");
		m->declare("filename", "gx-tonestack-roland.dsp");
	...
	virtual void instanceConstants(int sample_rate) {
		...
		fConst1 = GxTonestackRoland_faustpower2_f(fConst0);
		fConst2 = GxTonestackRoland_faustpower3_f(fConst0);
		fConst3 = 3.0 * fConst2;
	}
	...
	virtual GxTonestackRoland* clone() {
		return new GxTonestackRoland(*this);
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		double fSlow2 = 2.7256800000000006e-07 * fSlow1;
		double fSlow3 = fConst1 * (fSlow1 * (6.8142000000000025e-06 * fSlow0 + -7.876920000000001e-07 - fSlow2) + 2.851440000000001e-05 * fSlow0 + 1.4234760000000002e-06);
		double fSlow4 = 1.6641900000000002e-09 * fSlow0;
		double fSlow5 = 4.724676000000001e-10 * fSlow0 - 1.8898704000000002e-11 * fSlow1;
		double fSlow6 = fConst2 * (fSlow1 * (fSlow5 + -4.7668896000000004e-11) + fSlow4 + 6.656760000000001e-11);
		double fSlow7 = 0.00831 * fSlow0;
		double fSlow8 = 0.0008200000000000001 * fSlow1;
		double fSlow9 = fConst0 * (fSlow8 + fSlow7 + 0.005107400000000001);
		...
		double fSlow16 = fSlow1 * (fSlow5 + 1.8898704000000002e-11) + fSlow15 * (fSlow4 + 6.656760000000001e-11 * (1.0 - fSlow1));
		double fSlow17 = fConst2 * fSlow16;
		double fSlow18 = fConst1 * (2.829e-07 * fSlow15 + fSlow1 * (3.2176800000000005e-07 - fSlow2) + fSlow0 * (6.8142000000000025e-06 * fSlow1 + 7.779000000000002e-07) + 3.1116000000000005e-08);
		double fSlow19 = fConst0 * (fSlow7 + fSlow8 + 6e-05 * fSlow15 + 0.00033240000000000006);
