...
Compilation options: -lang cpp -i -fpga-mem-th 4 -ct 1 -cn GxTonestackIbanez -scn chdsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -double -ftz 0
------------------------------------------------------------ */

#ifndef  __GxTonestackIbanez_H__
#define  __GxTonestackIbanez_H__
...
#ifndef FAUSTCLASS 
#define FAUSTCLASS GxTonestackIbanez
#endif
...
static double GxTonestackIbanez_faustpower2_f(double value) {
	return value * value;
}
static double GxTonestackIbanez_faustpower3_f(double value) {
	return value * value * value;
}

class GxTonestackIbanez : public chdsp {
 ...
 public:
	...
	GxTonestackIbanez(const GxTonestackIbanez&) = default;
	
	virtual ~GxTonestackIbanez() = default;
	
	GxTonestackIbanez& operator=(const GxTonestackIbanez&) = default;
	
	void metadata(Meta* m) { 
		m->declare("chameleon_flags", "-double");
		m->declare("compile_options", "-lang cpp -i -fpga-mem-th 4 -ct 1 -cn GxTonestackIbanez -scn chdsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -double -ftz 0");
		m->declare("filename", "gx-tonestack-ibanez.dsp");
	...
	virtual void instanceConstants(int sample_rate) {
		...
		fConst1 = GxTonestackIbanez_faustpower2_f(fConst0);
		fConst2 = GxTonestackIbanez_faustpower3_f(fConst0);
		fConst3 = 3.0 * fConst2;
	}
	...
	virtual GxTonestackIbanez* clone() {
		return new GxTonestackIbanez(*this);
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("Ibanez");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		double fSlow2 = 4.0108000000000004e-07 * fSlow1;
		double fSlow3 = fConst1 * (fSlow1 * (1.0027e-05 * fSlow0 + -3.5719200000000006e-06 - fSlow2) + 0.00010263250000000001 * fSlow0 + 5.050300000000001e-06);
		double fSlow4 = 6.75e-09 * fSlow0;
		double fSlow5 = 9.45e-10 * fSlow0 - 3.78e-11 * fSlow1;
		double fSlow6 = fConst2 * (fSlow1 * (fSlow5 + -2.3219999999999998e-10) + fSlow4 + 2.7e-10);
		double fSlow7 = 0.025067500000000003 * fSlow0;
		double fSlow8 = 0.0004 * fSlow1;
		double fSlow9 = fConst0 * (fSlow8 + fSlow7 + 0.0150702);
		...
		double fSlow16 = fSlow1 * (fSlow5 + 3.78e-11) + fSlow15 * (fSlow4 + 2.7e-10 * (1.0 - fSlow1));
		double fSlow17 = fConst2 * fSlow16;
		double fSlow18 = fConst1 * (9.45e-07 * fSlow15 + fSlow1 * (4.2808000000000006e-07 - fSlow2) + fSlow0 * (1.0027e-05 * fSlow1 + 2.6324999999999998e-06) + 1.0530000000000001e-07);
		double fSlow19 = fConst0 * (fSlow7 + fSlow8 + 6.75e-05 * fSlow15 + 0.0010027);
